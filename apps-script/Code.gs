...
function doPost(e) {
  try {
    ...
    var subject = 'Quay 1 — Tax Invoice' + (invoices.length > 1 ? 's (' + invoices.length + ')' : ' ' + (invoices[0].doc || ''));
    ...
    var html =
      ...
      '<p>Please find ' + (invoices.length > 1 ? 'your attached tax invoices' : 'your attached tax invoice') +
    ...
    var plain = 'Good day ' + broker + ',\n\nPlease find your attached tax invoice' +
      (invoices.length > 1 ? 's' : '') + ' from Quay 1 International Realty.\n\nKind regards,\nQuay 1 International Realty';

    var draft = GmailApp.createDraft(to, subject, plain, { htmlBody: html, attachments: attachments, name: 'Quay 1 International Realty' });
